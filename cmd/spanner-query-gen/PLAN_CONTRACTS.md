...
same display-name classification as relational nodes, so subquery operators
such as Array Subquery and Scalar Subquery keep their concrete families;
expression nodes such as Reference, Function, Constant, and Parameter that
match no concrete family classify as `scalar` and never as `unknown`.
