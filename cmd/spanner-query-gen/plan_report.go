...
// The normalization of raw QueryPlans into contract-evaluator inputs lives
// in the lightweight plancontract package so it can be reused against plans
// obtained outside this command. These aliases keep the historical
// planReport* names used throughout this package and its tests.

func planReportOperators(plan *spannerpb.QueryPlan) []planReportOperator {
	return plancontract.NormalizeOperators(plan)
}

func planReportOperatorEdges(plan *spannerpb.QueryPlan) []planReportOperatorEdge {
	return plancontract.NormalizeOperatorEdges(plan)
}

func planReportOperatorTreeDigest(plan *spannerpb.QueryPlan) string {
	return plancontract.OperatorTreeDigest(plan)
}

func planReportClassificationWarnings(operators []planReportOperator) []planReportDiagnostic {
	return plancontract.ClassificationWarnings(operators)
}

func planReportOperatorFamilies(operators []planReportOperator) []string {
	return plancontract.ObservedOperatorFamilies(operators)
}

func planReportOperatorFamily(node *spannerpb.PlanNode) string {
	return plancontract.OperatorFamily(node)
}

func planReportOperatorMetadataString(node *spannerpb.PlanNode, key string) string {
	return plancontract.OperatorMetadataString(node, key)
}
