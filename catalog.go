...
func (c *Catalog) applyCreateTable(ddl *ast.CreateTable) error {
	...
	if len(table.PrimaryKey) == 0 {
		if existing, _ := table.Column("rowid"); existing != nil {
			return fmt.Errorf("table %s defines rowid without a primary key; Spanner reserves rowid for the generated hidden primary key", name)
		}
		table.Columns = append(table.Columns, &Column{
			...
			Hidden:       true,
			PrimaryKey:   true,
			GeneratedSQL: "GENERATED BY DEFAULT AS IDENTITY (BIT_REVERSED_POSITIVE)",
		})
		table.PrimaryKey = append(table.PrimaryKey, &KeyPart{Name: "rowid"})
	}
