...
| `FACTORIZED_MODE` | `FACTORIZE_LEFT`, `FACTORIZE_RIGHT`, `FACTORIZE_BOTH`; INNER JOIN with equality conditions only | Documented join hint (query-syntax Hints section, "Factorized mode") that deduplicates join-key values on one or both sides before the join; most relevant to many-to-many graph traversals. Verified accepted and functional on Omni 2026.r1-beta (2026-06-13): on a simple `Singers JOIN Concerts ON SingerId` the hint changes the plan — factorization appears as added `Aggregate` operators (Local Hash/Stream Aggregate) that dedup the join key, plus an `Array Unnest` re-expansion after the join, where the unhinted plan was a plain Distributed Cross Apply with no aggregate. Each hint value (`FACTORIZE_LEFT`/`BOTH`, with and without `JOIN_METHOD=HASH_JOIN`) produced a distinct operator-tree digest, so the hint is not silently ignored. There is no dedicated "factorize" display name or metadata flag; the dedup is inferred from the added aggregate/unnest shape. |
