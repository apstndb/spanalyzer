...
| `GROUPBY_SCAN_OPTIMIZATION` | `TRUE`, `FALSE` | Checked in table-hint probes. `TRUE` changed placement of `Distributed Union` around `Aggregate` in the synthetic probe. On a forced direct-GQL secondary index, TRUE selects `scan_method=Row`; FALSE is byte-identical to the unhinted `Automatic` control at v1-v8. |
| `SCAN_METHOD` | `AUTO` default, `BATCH`, `ROW`, `COLUMNAR`, `NO_COLUMNAR` | Checked in table-hint and statement-hint positions. Table `SCAN_METHOD=BATCH` can be rejected on the right side of apply joins and related shapes. The direct-GQL edge-element pair makes that boundary concrete: ROW succeeds v1-v8, while BATCH is rejected v1-v5 and succeeds on the `Collaborations` scan v6-v8. On a forced graph-node secondary index, COLUMNAR and NO_COLUMNAR are accepted and plan-visible v1-v8 as `Batch/Columnar` versus `Automatic/Row`; the exact no-hint control is `Automatic` with no `scan_format`. |
| `INDEX_STRATEGY` | `FORCE_INDEX_UNION` | Accepted for relational and direct-GQL disjunction shapes and produced `Union All` over two named index-scan branches. The GQL unhinted control uses one scan at v1-v6 and independently adopts the union plan at v7-v8. Rejected shapes returned `InvalidArgument` in broad statement-position probes. |
| `SEEKABLE_KEY_SIZE` | `0` to `16`; requires `FORCE_INDEX` | Checked with `FORCE_INDEX=_BASE_TABLE` and key sizes `0`, `1`, `2` on a two-part primary key. A larger value than the key length was separately checked and rejected. Statement-prefix use is rejected as unsupported; use table-hint position. Direct GQL graph elements also preserve sizes `0` and `1` on the Filter Scan while using the forced secondary index at v1-v8. |
...
| `JOIN_METHOD` | `HASH_JOIN`, `APPLY_JOIN`, `MERGE_JOIN`, `PUSH_BROADCAST_HASH_JOIN` | Checked across direct `INNER`, `LEFT`, `RIGHT` joins and subquery predicates (`IN`, `EXISTS`, `NOT IN`, `NOT EXISTS`) through statement hints. Push broadcast is rejected in optimizer versions 1 and 2, accepted from version 3. Direct GQL traversal MERGE is accepted and visible from v1, while direct and statement GQL PUSH keep the v2/v3 boundary. Official node-to-edge, subpath-to-edge, between-path, and later-MATCH placements are plan-visible; pinned Omni also honors two grammar-admitted placements that current graph docs say are not allowed, so those remain runtime-extension evidence. Statement-level HASH/MERGE/APPLY/PUSH affect graph arms of the tested GQL INTERSECT; none is a direct set-operator hint. GQL VALUE rejects JOIN_METHOD itself. |
| `HASH_JOIN_BUILD_SIDE` | `BUILD_LEFT`, `BUILD_RIGHT`; only with `JOIN_METHOD=HASH_JOIN` | Included in join and subquery matrices. This can fix build/probe orientation for hash joins when `JOIN_METHOD=HASH_JOIN` is already selected. Direct GQL traversal left/right choices are plan-distinct at v1-v8 and reverse Build/Probe topology without a dedicated metadata value. |
| `BATCH_MODE` | `TRUE` default, `FALSE`; only with `JOIN_METHOD=APPLY_JOIN` | `TRUE` produces distributed apply forms for several join/subquery shapes. `FALSE` keeps row-at-a-time apply forms in checked shapes. Direct GQL traversal TRUE/FALSE is accepted v1-v8 and yields two versus one Distributed Cross Apply operators; TRUE matches the unhinted choice at v6-v8. GQL VALUE rejects BATCH_MODE. Some broad statement-position combinations returned `Hint batch_mode results in no plan.` |
| `HASH_JOIN_EXECUTION` | `MULTI_PASS` default, `ONE_PASS`; only with `JOIN_METHOD=HASH_JOIN` | Included in join and subquery matrices. The observed `PlanNode` metadata does not distinguish `MULTI_PASS` from `ONE_PASS`, so PLAN can verify only that a hash join was selected. It cannot verify the requested value or runtime spill/scan-count behavior. On correlated GQL EXISTS, MULTI_PASS and accepted ONE_PASS are byte-identical to the unhinted plan and select no Hash Join; ONE_PASS keeps the v3/v4 acceptance boundary but has no plan-visible effect in that placement. |
| `FACTORIZED_MODE` | `FACTORIZE_LEFT`, `FACTORIZE_RIGHT`, `FACTORIZE_BOTH`; INNER JOIN with equality conditions only | All three values were accepted on projection-eligible `Albums`/`Songs` inner equijoins at optimizer versions 1-8. Without a forced join method, the factorization signature (Local Stream Aggregate with variable-bearing Key/Agg links plus Array Unnest) was absent in v1-v4 and visible in v5-v8. Explicit `HASH_JOIN, FACTORIZE_BOTH` made both factorized sides visible in v1-v8. Join-key-only and non-equality forms were accepted without visible effect in v1-v4, then rejected as ineligible in v5-v8; LEFT OUTER was rejected throughout. Quantified and documented nonlinear GQL `FACTORIZE_BOTH` pairs independently repeat the exact v4/v5 hinted/control equality transition. No dedicated factorized metadata exists, and PLAN supplies no performance evidence. See the dedicated observations. |

Details:

- [Join Hint Matrix](QUERY_EXECUTION_OPERATORS_OBSERVATIONS.md#join-hint-matrix)
- [Subquery Predicate Statement-Hint Matrix](QUERY_EXECUTION_OPERATORS_OBSERVATIONS.md#subquery-predicate-statement-hint-matrix)
- [Factorized-mode plan observations](FACTORIZED_MODE_OBSERVATIONS_2026-08-11.md)
- [GQL hint and optimizer-version observations](GQL_HINT_VERSION_OBSERVATIONS_2026-08-11.md)
